...
#pragma warning restore CS0067
        ...
        static readonly Callback sceneOperationCallback = Callback.Before(Phase.OpenCallbacks).Do(ResolveAllScenes);
        ...
        static void Upgrade()
        {
            ...
            if (SceneManager.settings.project.sceneData?.Count() == 0)
                return;

            AssetDatabase.StartAssetEditing();

            var scenes = Enumerate<SceneReferenceCollection>(Key);
            var list = new List<Scene>();

            foreach (var (scene, references) in scenes)
            {

                foreach (var reference in references.references)
                    scene.AddCrossSceneReference(reference);
            ...
            SceneManager.settings.project.sceneData = null;
        ...
        {
            if (SceneManager.settings.project.sceneData is not null)
                foreach (var scene in SceneManager.settings.project.sceneData)
                    if (scene.Value.Get(key, out var json) && Scene.TryFind(scene.Key, out var targetScene) && TryDeserialize<T>(json, out var t))
