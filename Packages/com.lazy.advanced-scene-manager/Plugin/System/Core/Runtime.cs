        ...
        SceneCollection.IMethods_Target
    {
        ...
        static void OnInitialize() =>
            SceneManager.OnInitialized(() =>
            {
                ...
                SceneManager.runtime.CheckIfCollectionIsActuallyOpen();

            });

        void CheckIfCollectionIsActuallyOpen()
        {
            ...
            bool IsOpen()
            {
                ...
                foreach (var scene in m_openCollection.scenesToAutomaticallyOpen)
                {
                    if (!scene.isOpen)
                        return false;
                }
        ...
        public SceneOperation OpenAdditive(IEnumerable<SceneCollection> collections, SceneCollection activeCollection = null, Scene loadingScene = null)
        {
            ...
            if (activeCollection)
                operation.Callback(SceneManager.runtime.TrackCollectionCallback(activeCollection, true));
        ...
        Callback[] TrackCollectionCallback(SceneCollection collection, bool isAdditive = false)
        {
            return new[]
            {
                Callback.After(Phase.UnloadScenes).Do(() =>
                {
                    Untrack(openCollection, isAdditive);
                    if (!isAdditive)
                        Untrack(openCollection, true); //Make sure additive collection is removed when it is opened non-additively
                }),
                Callback.After(Phase.LoadScenes).Do(() =>
                {
