    ...
    static class ToolbarButton
    {
        ...
        static void DrawPlayButton(int i)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (!isContext)
                {
                    SceneManager.app.Restart(new Core.App.StartupProps(SceneManager.app.startupProps) { openCollection = collection, runStartupProcessWhenPlayingCollection = runStartupProcess });
                }
                else
                {
